...
class Data extends AbstractHelper
{
    ...
    public function getAltImgHtml($product, $imageTypeId = 'product_base_image', $w = null, $h = null)
    {
        ...
        if ($gal = $product->getMediaGalleryImages())
        {
            ...
            {
                // // TODO: searching by "order" column
                // $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/product.log');
                // $logger = new \Zend\Log\Logger();
                // $logger->addWriter($writer);
                //
                // $logger->info('image->getFile()=' . $image->getFile() ); ///
                // $logger->info('print: ' . print_r($image->getData(), 1) ); ///

                return '<img class="alt-img" src="' 
                    . $this->helperImage->getImgExtended($product, $imageTypeId, $w, $h, $image->getFile())
                    . '" alt="' . $product->getName() . '" />';
            }
        }
